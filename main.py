...
def xor_bity(a, b):
    # licze xor dwoch ciagow bitow o tej samej dlugosci
    wynik = []
...
def przesun_w_lewo(bity, o_ile):
    # przesuwam cyklicznie klucz w lewo
    o_ile = o_ile % len(bity)
    return bity[o_ile:] + bity[:o_ile]


def przesun_w_prawo(bity, o_ile):
    # przesuwam cyklicznie klucz w prawo
    o_ile = o_ile % len(bity)
    return bity[-o_ile:] + bity[:-o_ile]


def funkcja_f(klucz_rundy, prawa_6):
    # licze funkcje f dla mini des
    # krok 1: rozszerzam prawa polowke 6 -> 8 bitow wedlug permutacji
    rozszerzone_8 = "".join(prawa_6[int(indeks)] for indeks in PERMUTACJA)
    # krok 2: mieszam rozszerzona polowke z kluczem rundy
    po_xor = xor_bity(rozszerzone_8, klucz_rundy)
    # krok 3: dzielę wynik na dwa nibbles po 4 bity
    lewa_4 = po_xor[:4]
    prawa_4 = po_xor[4:]
    # krok 4: po SBOX-ach mam 3 + 3 bity, czyli wyjscie 6-bitowe
    return SBOX_1[int(lewa_4, 2)] + SBOX_2[int(prawa_4, 2)]


def szyfruj_blok(blok_12, klucz_8):
    # szyfruje pojedynczy blok 12 bitow
    lewa = blok_12[:6]
    prawa = blok_12[6:]

    for numer_rundy in range(1, LICZBA_RUND):
        # dla kazdej rundy biorę inny klucz przez przesuniecie
        klucz_rundy = przesun_w_lewo(klucz_8, numer_rundy)
        # schemat Feistela: lewa idzie na prawa, prawa licze przez XOR z F
        nowa_lewa = prawa
...
def deszyfruj_blok(blok_12, klucz_8):
    # deszyfruje pojedynczy blok 12 bitow
    lewa = blok_12[:6]
    prawa = blok_12[6:]

    for numer_rundy in range(1, LICZBA_RUND):
        # deszyfruje tym samym schematem, ale klucz rundy licze odwrotnie
...
def podziel_na_bloki(ciag_bitow):
    # dziele ciag bitow na pelne bloki i koncowke
...
def szyfruj_cbc(ciag_bitow, klucz_8, iv_12):
    ...
    for blok in bloki:
        # w cbc najpierw mieszam blok jawny z poprzednim szyfrogramem (lub iv)
        wejscie_rundy = xor_bity(blok, poprzedni)
        # dopiero taki blok szyfruje mini des
...
def deszyfruj_cbc(ciag_bitow, klucz_8, iv_12):
    # deszyfruje trybem cbc
    bloki, koncowka = podziel_na_bloki(ciag_bitow)
    poprzedni = iv_12
    wynik = []

    for blok in bloki:
        # najpierw deszyfruje biezacy blok szyfrogramu
        po_des = deszyfruj_blok(blok, klucz_8)
        # potem xor z poprzednim szyfrogramem odzyskuje tekst jawny
...
def szyfruj_ctr(ciag_bitow, klucz_8, licznik_start=0):
    # szyfruje i deszyfruje trybem ctr
    bloki, koncowka = podziel_na_bloki(ciag_bitow)
    wynik = []
    licznik = licznik_start

    for blok in bloki:
        # licznik koduje jako blok 12-bit, zeby pasowal do mini des
        blok_licznika = format(licznik % (2 ** ROZMIAR_BLOKU), "012b")
        # szyfrowany licznik daje strumien klucza dla aktualnego bloku
        strumien_klucza = szyfruj_blok(blok_licznika, klucz_8)
        # blok danych mieszam xor-em ze strumieniem klucza
...
def wczytaj_pnm_binarne(sciezka):
    # odczytuje p4 p5 p6 i oddzielam naglowek od pikseli
    ...
    while len(tokeny) < ile_tokenow:
        # pomijam biale znaki miedzy tokenami naglowka
        while i < len(surowe) and surowe[i] in b" \t\r\n":
            i += 1

        if i < len(surowe) and surowe[i] == 35:
            # znak # oznacza komentarz w pnm, wiec ide do konca linii
            while i < len(surowe) and surowe[i] != 10:
                i += 1
            continue

        # czytam kolejny token (np. szerokosc, wysokosc, max wartosc)
        start = i
...
def wczytaj_obraz(sciezka):
    # obsluguje pnm lub png jpg przez pillow
    ...
    # normalizuje tryb, zeby zapis po modyfikacji byl stabilny
...
def zapisz_obraz(meta, sciezka, nowe_piksele):
    # zapisuje obraz po podmianie danych pikseli
    if meta[0] == "pnm":
        _, naglowek, _, _ = meta
...
def main():
    # pobieram argumenty i uruchamiam caly eksperyment
    ...
    # waliduje format klucza i iv zanim rusze przetwarzanie obrazu
    sprawdz_format_bitowy(args.klucz, 8, "klucz")
    sprawdz_format_bitowy(args.iv, 12, "iv")

    # wczytuje obraz i wyciagam same dane pikseli
    ...
    # przygotowuje raport z wnioskami i metrykami bit-flipping
    raport = []
    ...
    for tryb in ("ecb", "cbc", "ctr"):
        # dla kazdego trybu robie: szyfrowanie -> atak 1 bit -> deszyfrowanie po ataku
        if tryb == "ecb":
        ...
        # zapisuje trzy obrazy wymagane w zadaniu
        ...
        # mierze ile bitow realnie sie zmienilo po ataku
